...
string Optimization(string as_code)
	{
		std::istringstream f(as_code);
		std::string line;
		string prev_line ="";
		getline(f, prev_line);
		std::string code = "";
		...
		string line_1 = prev_line;
		string line_2 = line;
		//printf("%s\n", line_1.c_str());
		// printf("%s\n\n", line_2.c_str());
		vector<string> line1_all,line2_all;

		stringstream ss1(line_1);
		string word1;
		while (ss1 >> word1) {
		    line1_all.push_back(word1);
		}
		string word2;
		stringstream ss2(line_2);
		while (ss2 >> word2) {
		    line2_all.push_back(word2);
		}


		int flag = 0;

		if((line1_all.size() == line2_all.size()) && (line2_all.size() == 3))
		{

		     if((line1_all[0] == "mov" )&&( line2_all[0] == "mov" ))
		    {

			if((line2_all[2] + string(",")) == line1_all[1] )
			    {

				if(line2_all[1] == (line1_all[2]+string(",")))
				{
				    flag = 1;
				}
			    }
...
start : program
	{
		fprintf(fp1,"\nLine %d: start : program\n",lineCount);
 		...
 		//if(errorCount == 0) {
                /* assembly code generation */string f_code=".MODEL SMALL\n.STACK 100H\n";
		 		
		 		f_code+=".DATA\n";
		 		
		 		//variables
		data_seg.push_back("address dw ?");
		data_seg.push_back("ret_val dw ?");
                for(int i=0; i<data_seg.size(); i++) {
                    f_code +="\t"+(string)data_seg[i]+"\n";
                }
			   ...
			   f_code+="\tcmp ax, 0     \n";
			   f_code+="\tjge start        ;jump if positive number \n";

			   f_code+="\tpush ax                        \n";

			   f_code+="\tmov ah, 2                      \n";
			   f_code+="\tmov dl, \"-\"    ;print '-' otherwise        \n";        
			   f_code+="\tint 21h                        \n";
			   ...
			   f_code+="\txor cx, cx    ;clearing register                \n";
			   f_code+="\tmov bx, 10    ;initalize bx by 10                 \n";

			   f_code+="\toutput_:                       \n";
			     f_code+="\t\txor dx, dx  ;clear dx                \n";
			     f_code+="\t\tdiv bx      ;ax = ax/bx (10)               \n";
			     f_code+="\t\tpush dx     ;pushing remainder into stack           \n";      
			     f_code+="\t\tinc cx      ;to count number of digit              \n";   
			     f_code+="\t\tor ax, ax                   \n";
			   f_code+="\tjne output_    ;loop until zf not 1                \n";

			   f_code+="\tmov ah, 2                     \n";

			   f_code+="\tdisplay:                      \n";
			     f_code+="\t\tpop dx       ;printing remainder from reverse order  \n";             
			     f_code+="\t\tor dl, 30h                  \n";
			     f_code+="\t\tint 21h                     \n";
			   f_code+="\tloop display                  \n";
			   ...
			   f_code+="\tpush address\n"; 	
			   f_code+="\tret\n";
		 	   f_code+="PRINT_ID ENDP\n\n";
				
 			   f_code+= $1->code;	
 		

                //f_code += (string)"end main";

                $$->code = f_code;
               
               	fprintf(as,"%s",f_code.c_str());
 		fprintf(opt_as,"%s",Optimization(f_code).c_str());
