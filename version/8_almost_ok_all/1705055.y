...
start : program
	{
		...
		data_seg.push_back("ret_val dw ?");
			   ...
			   init+="\tcmp ax, 0     \n";
			   init+="\tjge start        ;jump if positive number \n";

			   init+="\tpush ax                        \n";

			   init+="\tmov ah, 2                      \n";
			   init+="\tmov dl, \"-\"    ;print '-' otherwise        \n";        
			   init+="\tint 21h                        \n";
			   ...
			   init+="\txor cx, cx    ;clearing register                \n";
			   init+="\tmov bx, 10    ;initalize bx by 10                 \n";

			   init+="\toutput_:                       \n";
			     init+="\t\txor dx, dx  ;clear dx                \n";
			     init+="\t\tdiv bx      ;ax = ax/bx (10)               \n";
			     init+="\t\tpush dx     ;pushing remainder into stack           \n";      
			     init+="\t\tinc cx      ;to count number of digit              \n";   
			     init+="\t\tor ax, ax                   \n";
			   init+="\tjne output_    ;loop until zf not 1                \n";

			   init+="\tmov ah, 2                     \n";

			   init+="\tdisplay:                      \n";
			     init+="\t\tpop dx       ;printing remainder from reverse order  \n";             
			     init+="\t\tor dl, 30h                  \n";
			     init+="\t\tint 21h                     \n";
			   init+="\tloop display                  \n";
