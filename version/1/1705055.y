...
vector<pair<string, string>> var_list;
...
var_declaration : type_specifier declaration_list  SEMICOLON
		...
		{
			printf("{%s %s} ", var_list[i].first.c_str(),var_list[i].second.c_str());
		}
		cout<<endl;
		string type_name = "";
		for(int i=0;i<var_list.size();i++)
		{
			...
			if(var_list[i].second == "array") type_name  = $1->Name+"_array";
...
declaration_list : declaration_list COMMA ID
 		  ...
 		  var_list.push_back(make_pair($3->Name.c_str(), "normal"));
 		  ...
 		  var_list.push_back(make_pair($3->Name.c_str(), "array"));
 		  ...
 		  fprintf(fp1,"\n%s\n",$1->Name.c_str());
 		  var_list.push_back(make_pair($1->Name.c_str(), "normal"));
 		  ...
 		  {
		  var_list.push_back(make_pair($1->Name.c_str(), "array"));
...
factor	: variable 
	...
	if(sii && (sii->id_type == "func_definition"))
		...
		for(int i=0;i<l;i++)
		{
				if((temp_arg[i] != arg_list[i]) && (temp_arg[i] !="") && (arg_list[i] !=""))
				{
				
				
				

				fprintf(er,"\nError at line %d: %dth parameter type [should be %s , but found %s] mismatch with declaration in function call %s\n",lineCount,i+1,temp_arg[i].c_str(),arg_list[i].c_str(), $1->Name.c_str());
				fprintf(fp1,"\nError at line %d: %dth parameter type [should be %s , but found %s] mismatch with declaration in function call %s\n",lineCount,i+1,temp_arg[i].c_str(),arg_list[i].c_str(), $1->Name.c_str());
				errorCount++;
				break;
				}
	...
	char *temp= newTemp();
	data_seg.push_back(string(temp) +  " dw ?");
	$$->code = $1->code + (string)"\tmov ax, "+ $1->symbol +"\n\tmov "+string(temp)+", ax\n\tinc "+$1->symbol+"\n"; 
